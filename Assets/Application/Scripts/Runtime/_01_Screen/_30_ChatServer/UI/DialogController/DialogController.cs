...
namespace Template.Screens.ChatServerClasses.UI
{
	...
	public class DialogController : CancelableTask
	{
		...
		public DialogController( ChatServer owner ) : base( owner )
		{
			m_Owner = owner ;
		}
		...
		/// </summary>
		/// <param name="owner"></param>
		public void SetOwner( ChatServer owner )
		{
			base.SetOwner( owner ) ;
		...
		public bool StartupController()
		{
			if( m_DialogCanvas.ActiveSelf == false )
			{
				...
				{
					// モンスターの一時停止実行
//					SharedSpace.PauseHouseAnimation() ;
		...
		public void CleanupController()
		{
			if( m_DialogCanvas.ActiveSelf == true )
			{
				...
				if( count == 0 )
				{
					...
					// モンスターの一時停止解除
//					SharedSpace.ResumeHouseAnimation() ;
				}
			}
		}
