...
{
	/// <summary>
	/// Ｊｉｎｇｌｅクラス Version 2022/05/04 0
	/// </summary>
	public class Jingle : ExMonoBehaviour
	{
		private static Jingle	m_Instance ;
		internal void Awake()
		{
			m_Instance = this ;
		}
		...
		// 基本パス(環境に合わせて書き換える事)
		private const string m_Path = "Sounds/Jingle" ;
		...
		/// <summary>
		/// Ｊｉｎｇｌｅを再生する(同期版:ファイルが存在しない場合は失敗)
		/// </summary>
		...
		public static int Play( string path, float volume = 1.0f, float pan = 0, bool loop = false )
		{
			...
			// 再生する
			return AudioManager.Play( audioClip, loop, volume * GetVolume(), pan, 0, "Jingle" ) ;
		}

		/// <summary>
		/// Ｊｉｎｇｌｅを再生する(非同期版:ファイルが存在しない場合はダウンロードを試みる)
		/// </summary>
		/// <param name="path">ファイル名</param>
		/// <param name="volume">ボリューム係数(0～1)</param>
		/// <param name="pan">パン(-1=左～0=中～+1=右)</param>
		...
		public static async UniTask<int> PlayAsync( string path, float volume = 1.0f, float pan = 0, bool loop = false )
		{
			...
			// 再生する
			playId = AudioManager.Play( audioClip, loop, volume * GetVolume(), pan, 0, "Jingle" ) ;
			if( playId <  0 )
			{
				// 失敗
		...
		/// <summary>
		/// ３Ｄ空間想定でＪｉｎｇｌｅを再生する(同期版:ファイルが存在しない場合は失敗)
		/// </summary>
		/// <param name="path">ファイル名</param>
		...
		/// <summary>
		/// ３Ｄ空間想定でＪｉｎｇｌｅを再生する(非同期版:ファイルが存在しない場合はダウンロードを試みる)
		/// </summary>
		/// <param name="path">ファイル名</param>
		/// <param name="position">音源の位置(ワールド座標系)</param>
		/// <param name="listener">リスナーのトランスフォーム</param>
		...
		/// <summary>
		/// 識別子で指定するＪｉｎｇｌｅが再生中か確認する(一時停止中でも再生扱いになる)
		/// </summary>
		/// <param name="playId">発音毎に割り当てられるユニークな識別子</param>
		/// <returns></returns>
		...
		/// <summary>
		/// 識別子で指定するＪｉｎｇｌｅが再生中か確認する(一時停止中は停止扱いになる)
		/// </summary>
		/// <param name="playId">発音毎に割り当てられるユニークな識別子</param>
		/// <returns></returns>
		...
		{
			return AudioManager.StopAll( "Jingle" ) ;
		}
		...
		{
			return AudioManager.PauseAll( "Jingle" ) ;
		}
		...
		{
			return AudioManager.UnpauseAll( "Jingle" ) ;
		}
