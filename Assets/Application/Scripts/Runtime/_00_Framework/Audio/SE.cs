...
namespace Template
{
	...
	public class SE : ExMonoBehaviour
	{
		...
		public const string	Decision	= "SE_Decision" ;
		public const string	Cancel		= "SE_Cancel" ;
		public const string	Selection	= "SE_Selection" ;
		...
		public static async UniTask LoadAsync()
		{
			...
			await Asset.LoadAssetBundleAsync( m_Path ) ;
		...
		public static int Play( string path, float volume = 1.0f, float pan = 0, bool loop = false )
		{
			...
			// 再生する
			return AudioManager.Play( audioClip, loop, volume * GetVolume(), pan, 0, "SE" ) ;
		}
		...
		public static async UniTask<int> PlayAsync( string path, float volume = 1.0f, float pan = 0, bool loop = false )
		{
			...
			// 再生する
			playId = AudioManager.Play( audioClip, loop, volume * GetVolume(), pan, 0, "SE" ) ;
			if( playId <  0 )
			{
				// 失敗
